                        ...
                        <table className="w-full">
                            <thead className="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-center text-xs font-semibold text-slate-600 uppercase">Imágenes</th>
                                    <th className="px-6 py-4 text-center text-xs font-semibold text-slate-600 uppercase">Logo</th>
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {filteredProducts.map((product) => {
                                    ...
                                    const createdDate = product.createdAt 
                                        ? new Date(product.createdAt.seconds * 1000).toLocaleDateString('es-MX', {
                                            year: 'numeric',
                                            month: 'short',
                                            day: 'numeric'
                                        })
                                    ...
                                    return (
                                        <tr key={product.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <div className="flex items-center justify-center gap-2">
                                                    {imageCount > 0 ? (
                                                        <>
                                                            ...
                                                            <span className="text-sm font-semibold text-green-700">{imageCount}</span>
                                                        </>
                                                    ) : (
                                            ...
                                            <td className="px-6 py-4 text-slate-600 text-sm">{createdDate}</td>
...
function EditProductModal({ product, onClose, onSave }: { product: FirebaseProduct; onClose: () => void; onSave: (product: FirebaseProduct) => void }) {
    ...
    const [productImages, setProductImages] = useState<string[]>(product.images?.map(img => img.url) || []);
    const [newImageUrl, setNewImageUrl] = useState('');
    const [uploadingImage, setUploadingImage] = useState(false);
    const [brandLogoUrl, setBrandLogoUrl] = useState(getBrandLogo(product.marca || ''));
    ...
    const loadSimilarProducts = async () => {
        ...
        console.log(`🔍 Buscando productos similares: marca="${product.marca}", tipo="${product.tipo}"`);
        ...
        try {
            ...
            const snapshot = await getDocs(q);
            console.log(`📦 Encontrados ${snapshot.docs.length} productos con misma marca y tipo`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            onSave({ ...product, ...updateData, images: productImages.map(url => ({ url })) });
    ...
    return (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[85vh] overflow-y-auto p-6">
                <h2 className="text-2xl font-bold text-slate-900 mb-4">Editar Producto</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Atributos (separados por coma)</label>
                        <textarea
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Medidas (separadas por coma)</label>
                        <textarea
                    ...
                    <div className="border-t-2 border-slate-200 pt-3 mt-4">
                        <h3 className="text-lg font-bold text-slate-900 mb-3">Imágenes del Producto</h3>
                        
                        {/* Imágenes actuales */}
                        {productImages.length > 0 && (
                            <div className="mb-3">
                                <p className="text-sm text-slate-600 mb-2">Imágenes actuales ({productImages.length})</p>
                        ...
                        {/* Subir imagen */}
                        <div className="mb-2">
                            <label className="block text-sm font-semibold text-slate-700 mb-1">Subir imagen</label>
                            <input
                                type="file"
                                accept="image/*"
                                onChange={(e) => {
                        ...
                        {/* Agregar por URL */}
                        <div className="mb-2">
                            <label className="block text-sm font-semibold text-slate-700 mb-1">O pegar URL</label>
                            ...
                            {!product.marca || !product.tipo ? (
                                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                                    <p className="text-sm text-amber-800">
                                        ℹ️ Necesitas especificar <strong>Marca</strong> y <strong>Tipo</strong> para ver recomendaciones
                                    </p>
                                </div>
                            ) : similarProducts.length > 0 ? (
                                <>
                                    <p className="text-xs text-slate-600 mb-2">
                                        Encontrados {similarProducts.length} productos con <strong>{product.marca}</strong> / <strong>{product.tipo}</strong>
                                    </p>
                                        ...
                                        {similarProducts.map((similar) => (
                                            similar.images?.map((img, idx) => (
                                                <button
                                                    ...
                                                    title={`${similar.original} (${similar.images?.length || 0} imágenes)`}
                                                >
                                                    <Image
                                                        src={img.url}
                                ...
                                <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                                    <p className="text-sm text-slate-600">
                                        No se encontraron productos con <strong>{product.marca}</strong> / <strong>{product.tipo}</strong> que tengan imágenes
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="border-t-2 border-slate-200 pt-3 mt-4">
                        <h3 className="text-lg font-bold text-slate-900 mb-3">Logo de la Marca</h3>
                        ...
                        {/* Subir logo */}
                        <div className="mb-2">
                            <label className="block text-sm font-semibold text-slate-700 mb-1">Subir logo</label>
                            <input
                                type="file"
                                accept="image/*"
                                onChange={(e) => {
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-1">O pegar URL del logo</label>
                            <div className="flex gap-2">
                                <input
                                    type="url"
