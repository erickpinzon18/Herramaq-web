    ...
    >
      ...
      <div className="p-4 bg-white">
        ...
        <p className="text-sm text-slate-500">
          <span className="font-semibold text-slate-700">{brand.count}</span>{" "}
          productos
        </p>
      </div>
      {isSelected && (
...
export default function ProductsPage() {
  ...
  // Cargar todas las marcas desde colección 'brands' (optimizado)
  const loadBrands = async () => {
    setLoadingBrands(true);
    console.log("🔄 Cargando marcas desde colección brands...");

    try {
      // Intentar cargar desde cache primero
      const cachedBrands = localStorage.getItem("herramaq_brands");
      const cacheTimestamp = localStorage.getItem("herramaq_brands_timestamp");
      ...
      if (cachedBrands && cacheTimestamp) {
        ...
        if (age < CACHE_DURATION) {
          ...
          console.log(`✅ Marcas cargadas desde cache: ${brands.length}`);
          setBrands(brands);
      ...
      // Si no hay cache válido, cargar desde Firestore
      const brandsRef = collection(db, "brands");
      const q = query(brandsRef, orderBy("productCount", "desc"));
      const snapshot = await getDocs(q);

      const brandsArray: BrandInfo[] = snapshot.docs
        .filter((doc) => {
          const data = doc.data();
          return (
            data.name !== "GENÉRICA" &&
      ...
      // Guardar en cache
      localStorage.setItem("herramaq_brands", JSON.stringify(brandsArray));
      localStorage.setItem("herramaq_brands_timestamp", Date.now().toString());

      console.log(
        `✅ Marcas cargadas desde Firestore: ${brandsArray.length} (${snapshot.docs.length} docs leídos)`
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <section
        ref={heroRef}
        className="relative w-full py-20 md:py-32 overflow-hidden bg-gradient-to-br from-slate-800 via-slate-900 to-blue-950"
      >
        <ACSpotlight
        ...
        <div className="relative z-10 w-full px-4 md:px-8 lg:px-16">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="hero-subtitle text-xl md:text-2xl text-slate-300 mb-12 leading-relaxed max-w-3xl mx-auto">
              Explora nuestras {brands.length} marcas de herramientas y
            ...
            <div className="search-bar max-w-2xl mx-auto">
              ...
              {!hasSearched && (
                <p className="mt-3 text-slate-400 text-sm">
                  💡 Las marcas se filtran automáticamente. Para buscar
                  productos, presiona <strong>Buscar</strong> o{" "}
                  <strong>Enter</strong>.
                </p>
              )}
              {hasSearched && (
      ...
      <section
        ref={statsRef}
        className="relative w-full py-16 bg-gradient-to-b from-slate-900 to-slate-800 overflow-hidden"
      >
            ...
            <div ref={productsGridRef}>
              {loadingProducts ? (
                <div className="text-center py-20">
                  <div className="inline-block animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-slate-700"></div>
                  <p className="mt-4 text-slate-600">
                    Cargando productos de {selectedBrand}...
                  </p>
                </div>
      ...
      <section className="relative w-full py-20 bg-gradient-to-r from-slate-800 to-slate-900 overflow-hidden">
        ...
        <div className="relative z-10 w-full px-4 md:px-8 text-center">
          ...
          <p className="text-xl text-slate-300 mb-8 max-w-2xl mx-auto">
            Contáctanos y te ayudaremos a encontrar la herramienta perfecta para
            tu proyecto
          </p>
